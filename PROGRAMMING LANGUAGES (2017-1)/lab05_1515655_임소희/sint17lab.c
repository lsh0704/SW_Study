...
void stmt(void)
{
	...
	if (thrown) { // 예외가 발생한 상황일 때
		...
		case LET:      //  Lab #4 블록 선언 & Lab #5 예외 정의
			match(LET);
			n = 0; //let 변수 및 예외 정의 선언한 개수 세기 위해
			while (token == INT | token == BOOL) //블록 선언
			{
				type = token;
				n++;
				declaration = 1;
			...
			while (token == EXC) //예외 정의
			{
				...
				while (token == ',') //변수 선언하는 경우
				{
					...
					if (token == EXC)
					{
						...
						declaration = 0;
						match(ID);
						symtable[loc].token = EXCID; //token ID->EXCID로 변경
					}
					if (token == INT || token == BOOL)
					{
						...
						declaration = 0;
						match(ID);
						symtable[loc].token = type; //token ID->INT || BOOL로 변경
						if (token == '=')
						{
							match('=');
							result = expr();
